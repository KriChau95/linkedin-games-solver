...
export function solve_tango(board: number[][], equals: Constraint[], crosses: Constraint[]){
    ...
    function next_place(r: number, c: number): Tuple{
        if (c == SIZE -1){
            return [r+1, 0];
        } else {
            return [r, c+1];
    ...
    function check_three_b2b(n: number, r: number, c: number){
        ...
        if (one_left == n && one_right == n) return true;
        if (two_left == n && one_left == n) return true;
        if (one_right == n && two_right == n) return true;
        if (one_above == n && one_below == n) return true;
        if (one_above == n && two_above == n) return true;
        if (one_below == n && two_below == n) return true;
    
        return false;

    }

    function check_equals_crosses(n: number, r: number, c: number){
